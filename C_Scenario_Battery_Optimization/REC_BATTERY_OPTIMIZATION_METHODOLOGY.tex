...
\textbf{Key Innovation:} Centralized REC-level optimization of heterogeneous distributed batteries (not a single centralized battery).
...
\begin{itemize}
    ...
    \item $\pi_t^{\text{DA}}$: Day-ahead market price at time $t$ [€/kWh]
...
\min Z = \sum_{i \in \mathcal{N}} \sum_{t \in \mathcal{T}} \Big[
    &P_{i,t}^{\text{grid,import}} \cdot \pi_t^{\text{DA}} \cdot \Delta t \\
...
\begin{equation}
E_{i,0}^{\text{SOC}} = E_i^{\text{cap}} \cdot \text{SOC}_i^{\text{init}} \quad \forall i \in \mathcal{B}
\end{equation}
...
\begin{enumerate}
    \item \textbf{Perfect Foresight:} Model assumes perfect day-ahead forecasts (unrealistic)
        \begin{itemize}
            \item \textbf{Mitigation:} Re-optimize with updated forecasts at intraday stage
        \end{itemize}
